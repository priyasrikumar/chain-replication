...
if __name__ == '__main__':
    results = open('duresults.csv', 'w')
    writer = csv.writer(results)
    writer.writerow(['chain length', 'duration', 'throughput'])
    configs = glob.glob('./340*.config')
    for configName in configs:
        ...
        writer.writerow(['{0}'.format(numServers), '{0}'.format(duration), '{0}'.format(throughput)])
        results.flush()
